package com.virtuos.Managers
{
	...
	public class OBJCollision
	{
		...
		public static function enemyCollisions(_numX:Number,_numY:Number,_to:String='R',_farm:BaseEnemy=null,_collisionDected:Boolean=false):Boolean{
			var i:uint=0;
			var _disX:Number;
			var _disY:Number;
		...
		/*	if(!DataManager._playerMoving)
			{
				/*if(_dis<DataManager._enemyCollision-DataManager._hunterTorchCollision/4&&!DataManager._playerDeath&&DataManager._veriation!=3)
				{
					_hunter._through=0;
					main3D._player.changePoint("+50");
					DispatchMananger._em3D.dispatchEvent(new MyEvents('feed',_hunter))
					return 'feed'
				}
				if(_dis>=DataManager._enemyCollision&&_dis<DataManager._hunterTorchCollision/2)
				{				
				...
				if(DataManager._veriation!=3){
					...
					if(_dis<DataManager._enemyCollision){
						_hunter._through=1
					}else if(_dis>DataManager._enemyCollision+_extend/2&&_hunter._through==1&&_dis<=DataManager._hunterTorchCollision){
						_hunter._through=2;
						
					}			
					if(_hunter._through==2){
						_hunter._through=0;
						main3D._player.changePoint("+10");
						return 'kill';
					}
